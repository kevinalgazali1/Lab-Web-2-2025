document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderAll() {
    ...
    renderDiscardPile(); //menampilkn kartu teratas di tumpukan buang
    updateHandTitles();
  }
  ...
  function renderDiscardPile() {
    const topCard = discardPile[discardPile.length - 1]; //menamppilkan kartu teratas dari tumpukan
    ...
    if (topCard.color === "Liar") { //kartu wild
  ...
  function playerPlayCard(card) { //cek giliran
    ...
    playerHand = playerHand.filter((c) => c !== card);//menghapus kartu yang baru dimainkan pemain
    discardPile.push(card);
    playerHasDrawnThisTurn = false;//resset status 
  ...
  function cpuTurn() {
    ...
    setTimeout(() => {
      ...
      if (cardToPlay) { //jika menemukan kartu valid
        cpuHand = cpuHand.filter((c) => c !== cardToPlay);
        discardPile.push(cardToPlay); //meletakkannya di tumpukan kartu
      ...
      } else {//tidak ada kartu yang valid
  ...
  function endRound(winner) {
    ...
    if (playerBalance <= 0) { //saldo pemain
