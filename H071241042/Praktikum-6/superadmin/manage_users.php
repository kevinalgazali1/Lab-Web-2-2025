...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_user'])) {
    ...
    $password = md5($_POST['password']); // boleh ganti password_hash() kalau mau lebih aman
    $role = mysqli_real_escape_string($conn, $_POST['role']);
    
    // kalau kosong → NULL, kalau ada → pastikan integer
    $project_manager_id = !empty($_POST['project_manager_id']) ? intval($_POST['project_manager_id']) : 'NULL';

    // perhatikan: tidak ada tanda kutip di sekitar $project_manager_id
